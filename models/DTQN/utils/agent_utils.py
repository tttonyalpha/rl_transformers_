...
from dtqn.networks import DTQN, DMQN, DM2QN, DRQN, DQN, ADRQN, DARQN, ATTN
...
def get_agent(
    ...
    elif "DMQN" in model_str:
        policy_net = make_dmqn(MODEL_MAP[model_str])
        target_net = make_dmqn(MODEL_MAP[model_str])
    elif "DM2QN" in model_str:
        policy_net = make_dmqn(MODEL_MAP[model_str])
        target_net = make_dmqn(MODEL_MAP[model_str])
    else:
    ...
    optimizer = optim.Adam(policy_net.parameters(), lr=float(learning_rate))
    ...
    elif MODEL_MAP[model_str] in (DTQN, ATTN, DMQN, DM2QN):
        agent = DtqnAgent(
            env,
            eval_env,
            policy_net,
