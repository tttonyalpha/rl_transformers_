...
def wandb_init(config, group_keys, **kwargs) -> str:
    ...
    # env_name = if config["env"] == 'Memory-5-v0' else config["env"]
    if config["env"] == 'Memory-5-v0':
        env_name = 'MemoryCards_' 
    elif config["env"] in ['AutoencodeEasy', 'RepeatPreviousEasy', 'RepeatPreviousMedium', 'RepeatPreviousHard']:
        env_name = 'POPGym_'
    else:
